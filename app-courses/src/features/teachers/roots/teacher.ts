...
export class Teacher {
    ...
    constructor(props: TeacherProps) {
        ...
        if (!/^https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)$/.test(props.linkedinUrl)) {
            throw "LinkedIn URL is not valid";
        }

        if (props.summary.trim().length < 10 || props.summary.trim().length > 2000) {
            throw "Summary must be between 10 and 2000 characters long";
        }

        if (props.photoUrl && !/^https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)\.(jpg|jpeg|png|gif)(\?[a-zA-Z0-9&=_-]*)?$/i.test(props.photoUrl)) {
            throw "Photo URL must be a valid image URL with jpg, jpeg, png, or gif extension";
        }

        if (props.skills.length < 2) {
            throw "Skills must have at least 2 items";
        }
    ...
    update(props: TeacherUpdateProps) {
        ...
        if (props.linkedinUrl && !/^https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_\+.~#?&//=]*)$/.test(props.linkedinUrl)) {
            throw "LinkedIn URL is not valid";
        }

        if (props.summary && (props.summary.trim().length < 10 || props.summary.trim().length > 2000)) {
            throw "Summary must be between 10 and 2000 characters long";
        }
